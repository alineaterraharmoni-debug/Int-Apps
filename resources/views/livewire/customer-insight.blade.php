<div>
    ...
    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 mb-5 grid grid-cols-2 md:grid-cols-5 gap-3 items-end">
        <div class="col-span-2 md:col-span-1">
            <label class="text-xs font-semibold text-gray-500 dark:text-gray-400 block mb-1">Cari nama</label>
            <input type="text" wire:model.live.debounce.400ms="search" placeholder="cth. Bank XYZ" class="w-full border border-gray-200 dark:border-gray-700 rounded-lg px-2.5 py-2 text-sm">
        </div>
        <div>
            <label class="text-xs font-semibold text-gray-500 dark:text-gray-400 block mb-1">Dari Tanggal</label>
            <input type="date" wire:model.live="date_from" class="w-full border border-gray-200 dark:border-gray-700 rounded-lg px-2.5 py-2 text-sm">
        </div>
        <div>
            <label class="text-xs font-semibold text-gray-500 dark:text-gray-400 block mb-1">Sampai Tanggal</label>
            <input type="date" wire:model.live="date_to" class="w-full border border-gray-200 dark:border-gray-700 rounded-lg px-2.5 py-2 text-sm">
        </div>
        <div>
            <label class="text-xs font-semibold text-gray-500 dark:text-gray-400 block mb-1">Urutkan</label>
            <select wire:model.live="sortBy" class="w-full border border-gray-200 dark:border-gray-700 rounded-lg px-2.5 py-2 text-sm">
    ...
    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-x-auto">
        <table class="w-full text-sm min-w-[760px] md:min-w-0">
            ...
            <tbody>
                ...
                @empty
                    <tr><td colspan="7" class="p-8 text-center text-gray-400 dark:text-gray-500">Belum ada customer. Klik "+ Customer Baru" buat mulai.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-2xl w-full max-w-lg max-h-[92vh] sm:max-h-[88vh] overflow-y-auto p-4 sm:p-6">
                ...
                <form wire:submit="save" class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea wire:model="address" rows="2" class="w-full border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm"></textarea>
                    </div>
                    <div>
